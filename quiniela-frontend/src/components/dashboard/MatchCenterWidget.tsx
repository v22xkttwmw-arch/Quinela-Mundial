...
function fmtDate(dt: string, locale: string): string {
  return toLocal(dt).toLocaleDateString(locale, {
...
function LiveBadge({ elapsed, t }: { elapsed: number | null, t: any }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-red-500/15 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-red-400 ring-1 ring-red-500/25">
      ...
      {t.live}{elapsed != null ? ` · ${elapsed}'` : ""}
    </span>
  );
}

// ── Tarjeta EN VIVO ────────────────────────────────────────────────────────────
function LiveCard({ match, t }: { match: Match, t: any }) {
  return (
    <div className={cn(
      ...
      <div className="relative flex items-center justify-between">
        <LiveBadge elapsed={match.elapsed} t={t} />
        {match.group_name && (
          <span className="text-[9px] font-bold text-slate-600">{t.grp} {match.group_name}</span>
        )}
      </div>
      ...
      <div className="relative flex items-center justify-between gap-2">
        ...
        <div className="flex flex-col items-center gap-0.5">
          ...
          <p className="text-[9px] uppercase tracking-widest text-slate-600">{t.score}</p>
        </div>
        <div className="flex flex-1 flex-col items-center gap-1.5">
...
function TodayCard({ match, t }: { match: Match, t: any }) {
      ...
      <div className="flex items-center justify-between">
        {isFT ? (
          ...
          <span className="rounded-full bg-blue-500/10 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-blue-400 ring-1 ring-blue-500/20">
            {t.today} · {fmtTime(match.kickoff_time, t.locale)}
          </span>
        )}
        {match.group_name && (
          <span className="text-[9px] font-bold text-slate-600">{t.grp} {match.group_name}</span>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex flex-col items-center">
          {isFT ? (
            ...
            <p className="text-lg font-light text-slate-600">{t.vs}</p>
          )}
        </div>
        <div className="flex flex-1 flex-col items-center gap-1.5">
          <TeamFlag team={match.away_team} />
...
function RadarRow({ match, t }: { match: Match, t: any }) {
  ...
  return (
    <div className={cn(
      ...
      <div className="w-20 shrink-0 text-right">
        <p className="text-[10px] font-semibold capitalize text-slate-300">{fmtDate(match.kickoff_time, t.locale)}</p>
        <p className="text-[11px] font-extrabold tabular-nums text-blue-400">{fmtTime(match.kickoff_time, t.locale)}</p>
      </div>
      ...
      <div className="flex flex-1 items-center justify-between gap-2 min-w-0">
        ...
        <span className="shrink-0 text-[10px] text-slate-600">{t.vs}</span>
      ...
      <div className="shrink-0 text-right hidden sm:block">
        ...
        {match.group_name && (
          <p className="text-[9px] font-bold text-slate-600">{t.grupo} {match.group_name}</p>
        )}
      </div>
    </div>
  );
...
function HistorialRow({ match, t }: { match: Match, t: any }) {
  ...
  return (
    <div className={cn(
      ...
      <div className="w-16 shrink-0">
        {live ? (
          ...
          <span className="text-[9px] font-bold uppercase tracking-wider text-slate-500">
            {match.status === "FT" ? t.finishedShort : match.status}
          </span>
        )}
      </div>
      ...
      {/* Hora */}
      <span className="shrink-0 text-[9px] text-slate-600">{fmtTime(match.kickoff_time, t.locale)}</span>
    </div>
  );
}
...
export function MatchCenterWidget() {
  ...
  const t = translations[language].matchCenter;
  ...
  const historialMatches = matches
    .filter((m) => {
      const tDate = toLocal(m.kickoff_time);
      return (m.status === "FT" && tDate >= cutoffHistory) || isLive(m.status);
    })
    ...
    <div className="space-y-5">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">
              {liveMatches.length > 0 ? t.liveToday : t.todayMatches}
            </p>
            ...
            {isRefreshing && (
              <span className="animate-pulse text-[9px] text-slate-700">{t.updating}</span>
            )}
          </div>
      ...
      {radarMatches.length > 0 && (
        <div className="space-y-2">
          ...
          <div className="space-y-1.5">
            {radarMatches.map((m) => <RadarRow key={m.id} match={m} t={t} />)}
          </div>
        </div>
      )}
      ...
      {historialMatches.length > 0 && (
        <div className="space-y-2">
          ...
          <div className="space-y-1.5">
            {historialMatches.map((m) => <HistorialRow key={m.id} match={m} t={t} />)}
          </div>
        </div>
      )}
